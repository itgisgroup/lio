...
const faqs = [
  ["Lio là gì?", "Lio là hệ sinh thái AI Agents của ERPCloud; LioDev là sản phẩm phát triển Odoo dùng chung Agent Core cho Desktop, Web và Headless automation."],
  ["Lio có sửa thẳng production không?", "Không. Agent chỉ tạo patch hoặc signed change set trong sandbox. Production cần test evidence và approval rõ ràng."],
  ["Lio có chạy local hoặc self-host không?", "Có. Kiến trúc provider-neutral và local-first cho phép đội ngũ kiểm soát môi trường, model và dữ liệu."],
  ["Lio khác editor AI phổ thông thế nào?", "LioDev hiểu model, view, security, workflow, dependency và tác động nghiệp vụ của Odoo."],
] as const
...
export default function App() {
  ...
  useEffect(() => { const path = window.location.pathname; const match = path.match(/^\/doc(\/vi)?(\/lo-la-gi)?\/?$/); if (match) { const locale = match[1] ? `/docs/vi${match[2] || ""}` : `/docs${match[2] || ""}`; window.location.replace(window.location.hostname === "localhost" ? `http://localhost:4321${locale}` : locale) } }, [])
  return <div className="site-shell"><ContactFab /><a className="docs-launch" href="/doc/vi/5-bai-toan-erp">5 bài toán ERP <ArrowRight size={14}/></a>
